...
import {
  ...
  Loader2, Eye, EyeOff, X, Image as ImageIcon, ChevronLeft, ChevronRight,
...
const DEFAULT_HERO: BlogHeroSettings = {
  ...
  title_bg: 'Фотосесии',
  subtitle_en: 'Behind the scenes of our most exciting photoshoots from the fashion world.',
  subtitle_bg: 'Зад кулисите на нашите най-вълнуващи фотосесии от света на модата.',
...
const blogHeroService = {
  async get(): Promise<BlogHeroSettings> {
    ...
    try { return JSON.parse(data.value); } catch { return DEFAULT_HERO; }
  },
...
const HeroEditorModal: React.FC<{
  ...
  const cls = "w-full bg-white/5 border border-white/10 text-white text-sm px-4 py-3 focus:outline-none focus:border-gold-accent/50 transition-colors placeholder:text-white/20";
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          <div className="space-y-3 p-4 bg-white/5 border border-white/5">
            <p className="text-[10px] uppercase tracking-widest text-gold-accent font-bold">Title Typography</p>
            <div className="grid grid-cols-1 gap-3">
              ...
              >
                {FONT_OPTIONS.map(opt => (
                  <option key={opt.value} value={opt.value} style={{ fontFamily: opt.value, background: '#111' }}>
                    {opt.name}
                  </option>
                ))}
              </select>
            ...
            <div className="grid grid-cols-2 gap-3">
              <input className={cls} placeholder="Title (EN)" value={form.title_en}
                onChange={e => setForm(f => ({ ...f, title_en: e.target.value }))} />
              <input className={cls} placeholder="Заглавие (BG)" value={form.title_bg}
                onChange={e => setForm(f => ({ ...f, title_bg: e.target.value }))} />
            </div>
          </div>

          {/* Subtitle Settings */}
          <div className="space-y-3 p-4 bg-white/5 border border-white/5">
            <p className="text-[10px] uppercase tracking-widest text-gold-accent font-bold">Subtitle Design</p>
            <div className="grid grid-cols-2 gap-3">
              <select className={cls} value={form.sub_font} onChange={e => setForm(f => ({ ...f, sub_font: e.target.value }))}>
                {FONT_OPTIONS.map(opt => <option key={opt.value} value={opt.value} style={{ fontFamily: opt.value, background: '#111' }}>{opt.name}</option>)}
              </select>
              <select className={cls} value={form.sub_color} onChange={e => setForm(f => ({ ...f, sub_color: e.target.value }))}>
                {COLOR_OPTIONS.map(opt => <option key={opt.value} value={opt.value} style={{ color: opt.value, background: '#111' }}>{opt.name}</option>)}
              </select>
            </div>
            ...
            <div className="space-y-4 pt-2">
               <div>
                  <div className="flex justify-between text-[9px] uppercase tracking-widest text-white/40 mb-1">
                    ...
                    <span>Opacity: {Math.round((form.sub_opacity || 0.5) * 100)}%</span>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <input type="range" min="12" max="32" step="1" className="accent-gold-accent" value={form.sub_size} onChange={e => setForm(f => ({ ...f, sub_size: Number(e.target.value) }))} />
                    <input type="range" min="0.1" max="1" step="0.1" className="accent-gold-accent" value={form.sub_opacity} onChange={e => setForm(f => ({ ...f, sub_opacity: Number(e.target.value) }))} />
                  </div>
               </div>
               <div>
                  <p className="text-[9px] uppercase tracking-widest text-white/40 mb-1">Weight</p>
                  <div className="flex gap-2">
                    {['300', '400', '600'].map(w => (
                      <button key={w} type="button" onClick={() => setForm(f => ({ ...f, sub_weight: w }))}
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <textarea className={cls + " min-h-[60px] resize-none"} placeholder="Subtitle (EN)" value={form.subtitle_en}
                onChange={e => setForm(f => ({ ...f, subtitle_en: e.target.value }))} />
              <textarea className={cls + " min-h-[60px] resize-none"} placeholder="Подзаглавие (BG)" value={form.subtitle_bg}
                onChange={e => setForm(f => ({ ...f, subtitle_bg: e.target.value }))} />
            </div>
          </div>
...
const DraggableImage: React.FC<{
  ...
  >
    ...
    <div className="absolute top-1 right-1 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
      {!isCover && onSetCover && (
        <button type="button" onClick={() => onSetCover(index)}
          className="p-1 bg-gold-accent/80 text-black" title="Set as cover">
          <ImagePlus className="w-3 h-3" />
        </button>
      )}
      <button type="button" onClick={() => onRemove(index)} className="p-1 bg-red-500/80 text-white">
        <X className="w-3 h-3" />
      </button>
    </div>
  </div>
...
const PostEditorModal: React.FC<{
  post?: BlogPost | null;
  onSave: (data: Partial<BlogPost>, files?: File[], bgFile?: File) => Promise<void>;
  onClose: () => void;
}> = ({ post, onSave, onClose }) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (duplicates.length > 0) {
      const truncate = (name: string) => name.length > 15 ? name.substring(0, 12) + '...' : name;
      
      if (duplicates.length === 1) {
        const msg = i18n.language === 'bg' 
          ? `Снимка "${truncate(duplicates[0])}" вече е добавена`
          : `Photo "${truncate(duplicates[0])}" is already added`;
        showToast(msg, 'error');
      } else {
        const displayCount = 2;
        const shown = duplicates.slice(0, displayCount).map(truncate).join(', ');
  ...
  const removeExisting = (idx: number) => setExistingImages(p => p.filter((_, i) => i !== idx));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      }, files.length > 0 ? files : undefined, bgFile || undefined);
      onClose();
    ...
    >
      ...
      >
        <div className="flex items-center justify-between p-6 border-b border-white/5">
          <h2 className="text-lg font-serif text-white">{post ? 'Edit Photoshoot' : 'New Photoshoot'}</h2>
          <button onClick={onClose} className="text-white/40 hover:text-white"><X className="w-5 h-5" /></button>
        </div>

        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input className={cls} placeholder="Title (EN)" value={form.title}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <textarea className={cls + " min-h-[80px] resize-none"} placeholder="Description (EN)" value={form.description}
              onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
            <textarea className={cls + " min-h-[80px] resize-none"} placeholder="Описание (BG)" value={form.description_bg}
          ...
          <div className="space-y-1">
            <p className="text-[9px] uppercase tracking-widest text-white/30">Tags (comma separated)</p>
            <textarea 
            ...
            <div className="flex gap-4 items-center">
              {bgPreview && (
                <div className="relative w-32 aspect-video group shrink-0">
                  <img src={bgPreview} className="w-full h-full object-cover" alt="" />
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    <button type="button" onClick={() => { setBgFile(null); setBgPreview(null); }}
                      className="p-1.5 bg-red-500/80 text-white rounded-full hover:bg-red-500">
                      <X className="w-3.5 h-3.5" />
                    </button>
                    <button type="button" onClick={() => { setImageToCrop(bgPreview!); setCropModalOpen(true); }}
              ...
              <label className="flex items-center gap-2 cursor-pointer text-white/50 hover:text-gold-accent transition-colors text-xs border border-dashed border-white/20 hover:border-gold-accent/50 px-4 py-3">
                <Upload className="w-4 h-4" /> {bgPreview ? 'Change' : 'Upload Cover'}
          ...
          <div className="border border-white/10 p-4 space-y-3">
            <p className="text-[10px] uppercase tracking-widest text-gold-accent font-bold flex items-center gap-2">
              ...
              <span className="text-white/30 font-normal ml-auto">Drag to reorder</span>
            </p>
            ...
            {previews.length > 0 && (
              <div>
                <p className="text-[9px] uppercase tracking-widest text-white/30 mb-2">New uploads ({previews.length})</p>
                <div className="grid grid-cols-5 md:grid-cols-6 gap-1.5">
                  {previews.map((src, i) => (
...
const Blog: React.FC = () => {
  ...
  const loadPosts = useCallback(async (showLoading = true) => {
    try {
      ...
      const data = isAdmin ? await blogService.getAllPosts() : await blogService.getPublishedPosts();
      setPosts(data);
  ...
  useEffect(() => {
    const ch = supabase
      .channel('blog_rt')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'blog_posts' }, () => loadPosts(false))
      .subscribe();
  ...
  /* ── Save post ── */
  const handleSavePost = async (data: Partial<BlogPost>, files?: File[], bgFile?: File) => {
    try {
      ...
      if (editingPost) {
        await blogService.updatePost(editingPost.id, { ...data, images, cover_image });
      ...
      if (err.code === '23505' && err.message?.includes('blog_posts_slug_key')) {
        showToast(
          isBg 
  ...
  const handleGridDragOver = (postId: string, overIdx: number) => {
    if (!gridDrag || gridDrag.postId !== postId || gridDrag.idx === overIdx) return;
  ...
  return (
    <div className="min-h-screen bg-background text-text-primary pb-20">
      ...
      <section className="relative min-h-[500px] md:h-[65vh] flex items-center justify-center overflow-hidden border-b border-white/5 pt-32 pb-20 md:pt-0 md:pb-0">
        <div className="absolute inset-0 z-0">
          {hero.background_image ? (
            <img src={hero.background_image} alt="" className="w-full h-full object-cover opacity-40 blur-[1px]" />
          ) : posts[0]?.cover_image ? (
            <img src={posts[0].cover_image} alt="" className="w-full h-full object-cover opacity-30 blur-[1px]" />
          ) : (
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 py-16 md:py-24">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 gap-6">
            ...
            <p className="text-[10px] uppercase tracking-[0.4em] text-gold-accent animate-pulse">Loading...</p>
          </div>
        ) : posts.length === 0 ? (
          ...
          <div className="space-y-24 md:space-y-32">
            {posts.map(post => {
              ...
              const MAX_PREVIEW = 8;
              const displayImages = isExpanded ? post.images : post.images.slice(0, MAX_PREVIEW);
                ...
                >
                  ...
                  {post.cover_image && (
                    <div className="relative w-full overflow-hidden mb-8 md:mb-10" style={{ aspectRatio: '3/1', maxHeight: '380px' }}>
                      ...
                      <img src={post.cover_image} className="absolute inset-0 w-full h-auto z-[1]" alt="" />
                  ...
                  <div className="mb-6 md:mb-8">
                        ...
                        <div className="flex flex-wrap items-center gap-4 md:gap-6 text-xs text-white/40 uppercase tracking-widest">
                          {post.location && (
                            <span className="flex items-center gap-1.5">
                              <MapPin className="w-3.5 h-3.5 text-gold-accent" /> {post.location}
                            </span>
                          )}
                          <span className="flex items-center gap-1.5">
                            <Calendar className="w-3.5 h-3.5 text-gold-accent" />
                            {new Date(post.date).toLocaleDateString(isBg ? 'bg-BG' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                          </span>
                          {!post.is_published && (
                            <span className="px-2 py-0.5 bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 text-[9px] font-bold">DRAFT</span>
                          )}
                        </div>
                      </div>
                    ...
                    {post.tags.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-2">
                        {post.tags.map(tag => (
                          <span key={tag} className="text-[10px] uppercase tracking-widest bg-white/5 border border-white/10 text-white/40 px-3 py-1">#{tag}</span>
                        ))}
                      </div>
                    )}
      ...
      <AnimatePresence>
        {editorOpen && (
          <PostEditorModal post={editingPost} onSave={handleSavePost}
            onClose={() => { setEditorOpen(false); setEditingPost(null); }} />
        )}
      </AnimatePresence>
      <AnimatePresence>
        {heroEditorOpen && (
          <HeroEditorModal settings={hero} onSave={handleSaveHero}
            onClose={() => setHeroEditorOpen(false)} />
        )}
      </AnimatePresence>
      <AnimatePresence>
        {lightbox && (
          <PhotoLightbox images={lightbox.images} startIndex={lightbox.index}
