...
const MaintenancePage: React.FC = () => {
  ...
  const handleNotify = (e: React.FormEvent) => {
    ...
    // Simulating API call since resend is not integrated yet
    setTimeout(() => {
      showToast(
        "Тази услуга е все още в разработка. Ще бъде достъпна скоро!",
        "info",
      );
      setEmail("");
      setIsLoading(false);
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (newData?.key === "maintenance_end_time")
            setEndTime(newData.value);
        },
      )
      .subscribe();
        ...
        <div className="w-full grid grid-cols-1 lg:grid-cols-[1fr,1px,1fr] gap-12 lg:gap-0 items-center">
          ...
          >
            ...
            <div className="flex gap-4 mb-12">
              {[
                ...
                <div key={i} className="flex flex-col items-center">
                  <div className="w-16 h-16 lg:w-20 lg:h-20 border border-white/20 rounded-xl flex items-center justify-center mb-2 backdrop-blur-xl bg-white/5 shadow-2xl relative overflow-hidden">
                    <AnimatePresence mode="wait">
                      {timeLeft ? (
                        <motion.span 
                          ...
                          className="text-2xl lg:text-3xl font-bold text-white tabular-nums relative z-10"
                        >
                        ...
                        <motion.div 
                          ...
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
                          className="w-8 h-1 bg-white/20 rounded-full animate-pulse"
                        />
                      )}
                    </AnimatePresence>
                  </div>
            ...
            <div className="w-full max-w-md mb-12">
              ...
              <form onSubmit={handleNotify} className="flex w-full">
                <input 
                  type="email" 
                  placeholder="Your email address"
                  className="flex-1 bg-white px-4 py-3 text-black text-sm outline-none rounded-l-sm transition-all focus:bg-white/90"
                  value={email}
                  disabled={isLoading}
                ...
                <button 
                  type="submit"
                  disabled={isLoading}
                  className="bg-black/80 hover:bg-black text-white px-6 py-3 text-xs uppercase font-bold tracking-widest border border-white/20 transition-all rounded-r-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
                  {isLoading ? (
                    <>
