          ...
          siteSettings.hero_media_type === "video" ? (
            <video
              ...
              className={`w-full h-full object-cover transition-opacity duration-1000 ${
                mediaLoaded ? "opacity-50" : "opacity-0"
              }`}
            >
              <source src={getOptimizedUrl(siteSettings.hero_media_url)} type="video/mp4" />
            </video>
          ) : (
            <div className={`w-full h-full transition-opacity duration-1000 ${mediaLoaded ? "opacity-50" : "opacity-0"}`}>
              <OptimizedImage
                key={siteSettings?.hero_media_url || "default"}
                src={(() => {
                  const heroUrl = siteSettings?.hero_media_url;
                  if (!heroUrl) return '/hero_desktop.webp';
                  return heroUrl;
                })()}
                mobileSrc={(() => {
                  const heroUrl = siteSettings?.hero_media_url;
                  if (!heroUrl) return '/hero_mobile.webp';
                  return heroUrl;
                })()}
                alt="Hero Background"
                priority={true}
