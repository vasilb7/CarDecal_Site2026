...
function findFile(slug) {
    // Exact match
    if (availableFiles.includes(slug + '.jpg')) return slug + '.jpg';
    
    // Normalize slug (handle 5x33 variations)
    const norm = slug.replace('5x33-', '5x33cm-');
    if (availableFiles.includes(norm + '.jpg')) return norm + '.jpg';
    
    // Handle leading zero variations
    const parts = slug.split('-');
    const num = parseInt(parts[1]);
    const variants = [
        `${parts[0]}-${num}.jpg`,
        `${parts[0]}-${num.toString().padStart(2, '0')}.jpg`,
        `${parts[0]}cm-${num}.jpg`,
        `${parts[0]}cm-${num.toString().padStart(2, '0')}.jpg`
    ];
    
    for (const v of variants) {
        if (availableFiles.includes(v)) return v;
    }
...
for(let i=0; i<lines.length; i+=2) {
    ...
    const bestFile = findFile(baseSlug);
...
fs.writeFileSync('e:/Antigravity/CarDecal3/data/models.ts', fileContent);
console.log('Final models.ts generated with ' + tsObjects.length + ' products.');
const missing = tsObjects.filter(o => o.avatar === '/LOGO.png').map(o => o.slug);
console.log('Missing images for: ' + missing.length + ' products: ' + missing.join(', '));
