...
const Header: React.FC = () => {
    ...
    // CENTRALIZED LOGO SIZE CONTROL
    // Adjust height values here (e.g., h-8, h-10, h-12) to change logo size globally
    ...
    const navLinks = (
        <>
            ...
            <NavLink to="/contributions" className={navLinkClasses}>{t('nav.contributions')}</NavLink>
            ...
            {isMenuOpen && (
                <div className="md:hidden bg-surface">
                    <nav className="flex flex-col items-center space-y-6 py-8">
                        ...
                        <Link to="/contact" className="btn-mobile-full mt-4 px-6 py-2 border border-gold-accent text-gold-accent text-sm uppercase tracking-widest hover:bg-gold-accent hover:text-background transition-colors duration-300">
                            {t('nav.book_now')}
                        </Link>
                    </nav>
                </div>
